...
import { AquariumApi } from "./api/aquariumApi.js";
...
const initialAquarium = {
  ...
  lightOn: true,
  filterWear: 82,
  feedingHours: [11, 16, 21], // Жорстко фіксовані години
  nextFeeding: null,
  foodLevel: 54,
...
export function useAquariumLogic() {
  ...
  useEffect(() => {
    ...
    function calculateNextFeeding() {
      ...
      // Гарантуємо, що години годування завжди фіксовані: 11, 16, 21
      const hoursList = [11, 16, 21]; 
      ...
      // Якщо всі години на сьогодні минули — ставимо 11:00 на завтра
    ...
    async function syncFromServer() {
      try {
        ...
        if (response.success) {
          // Зберігаємо старі стабільні години годування перед копіюванням даних сервера
          const fallbackHours = aquarium.feedingHours;
          
          Object.assign(aquarium, response.data);
    ...
    function updateFeedingCountdown() {
      // Якщо дата ще не проініціалізована або не валідна, розраховуємо локально
      if (!aquarium.nextFeeding || isNaN(new Date(aquarium.nextFeeding).getTime())) {
        calculateNextFeeding();
      }
